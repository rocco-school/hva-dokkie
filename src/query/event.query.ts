export const EVENT_QUERY: {
    ...
    UPDATE_EVENT: "UPDATE event SET description = ?, eventStatus = ? WHERE eventId = ?",
    DELETE_EVENT: "DELETE FROM event WHERE eventId = ?",
    SELECT_EVENTS_BY_USER: "SELECT event.eventId, event.description, event.dateCreated, event.eventStatus FROM participant INNER JOIN event ON participant.eventId = event.eventId WHERE userId = ?",
